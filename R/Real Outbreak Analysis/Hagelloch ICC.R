# Estimates of the measles outbreak in Hagelloch using ICC interval


library(fdrtool)


icc <- function(data, N = 1000){
  ...
  data.frame(mu=mu, sigma=sigma, w1=w1, w2=w2, w3=w3, w4=w4, w5=w5, w6=w6, w7=w7, msg = "Successful convergence")
}


# define the density
getdens <- function(data, par, weights) {
  mu <- par[1]; sigma <- par[2]
  w1 <- weights[1]; w2 <- weights[2]; w3 <- weights[3]; w4 <- weights[4]
  w5 <- weights[5]; w6 <- weights[6]; w7 <- weights[7]
  ...
  for(l in 1:length(data)){
    ...
    fx[l] <- w1*d1 + w2*d2 + w3*d3 + w4*d4 + w5*d5 + w6*d6 + w7*d7
  }
  
  data.frame(x = data, f = fx)
  
}


## *** Estimates using ICC interval ****
## *************************************
#load("./data/si_measles.rda")


#icc_hag <- icc(measles$hagelloch)
#save(icc_hag, file = "./results/outbreak analysis/measles_icc.rda")
